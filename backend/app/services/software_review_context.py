...
class SoftwareReviewContextBuilder:
    def __init__(
        self,
    ...
    def build(self, review_id: str, test_mode: bool = False) -> SoftwareAgentContext:
        ...
        feature_categories = features.get("category") if isinstance(features, dict) else []
        categories = self._reviews.get_category_names(self._read_string_list(feature_categories))
        software_name_map = self._reviews.get_software_names_by_ids(
            self._read_string_list(review.get("integrate_software"))
            + self._read_string_list(review.get("used_software"))
        )

        reviewer = self._build_reviewer_context(
            review,
            user,
        ...
        payload = {
            "subject": {
                "name": self._text(review.get("software_name")),
                "slug": self._text(review.get("software_slug")),
                "category_labels": categories,
            },
            "reviewer": reviewer,
            "review_content": {
                "headline": self._text(review.get("title")),
                "body": self._text(review.get("summary")),
                "strength": self._textish(review.get("strength")),
                "weakness": self._textish(review.get("weakness")),
                "ratings": {
                    "ease_of_use": self._parse_int(review.get("ease_of_use")),
                    "features_functionality": self._parse_int(review.get("features_functionality")),
                    "customer_support": self._parse_int(review.get("customer_support")),
                    "overall": self._parse_int(review.get("overall")),
                },
            },
            "usage": usage,
            "signals": signals,
        ...
        metadata = {
            "review_id": str(review.get("_id", review_id)),
            "status_code": self._parse_int(review.get("is_active")),
            "status_label": self._status_label(review.get("is_active")),
            "step": self._parse_int(review.get("step")),
            "submitted_by": submitted_by,
            "rejection_reason": self._text(review.get("reason")) or None,
            "request_token": self._text(review.get("requesttoken")) or None,
    ...
    def _validate_review(self, review: dict[str, Any], test_mode: bool) -> None:
        submitted_by = self._parse_int(review.get("submitted_by"))
        if submitted_by != 1:
            raise ValueError("Software review is not eligible: submitted_by must be 1 for current workflow")

        is_active = self._parse_int(review.get("is_active"))
        if not test_mode and is_active != 0:
            raise ValueError("Software review is not eligible: is_active must be 0")

        step = self._parse_int(review.get("step"))
        if step is None or step < 2:
            raise ValueError("Software review is not eligible: step must be >= 2")

        required_text_fields = ("title", "summary", "strength", "weakness")
        for field_name in required_text_fields:
            if not self._textish(review.get(field_name)):
                raise ValueError(f"Software review is not eligible: {field_name} is required")

        for rating_name in ("ease_of_use", "features_functionality", "customer_support", "overall"):
            rating_value = self._parse_int(review.get(rating_name))
            if rating_value is None or rating_value < 1 or rating_value > 5:
                raise ValueError(f"Software review is not eligible: {rating_name} must be between 1 and 5")
    ...
    def _build_usage_context(self, review: dict[str, Any], software_name_map: dict[str, str]) -> dict[str, Any]:
        return {
            "duration_value": self._parse_int(review.get("use_in_time")),
            "duration_unit": self._text(review.get("use_time_format")).lower(),
            "frequency": self._text(review.get("frequent_use")).lower(),
            "pricing": self._text(review.get("software_pricing")).lower(),
            "integrates_other_software": self._text(review.get("is_integrated")).lower(),
            "integrated_software": self._resolve_name_list(review.get("integrate_software"), software_name_map),
            "switched_from_other_software": self._text(review.get("switched_from")).lower(),
            "used_software_before_switch": self._resolve_name_list(review.get("used_software"), software_name_map),
        }
    ...
    def _build_prechecks(
        ...
        *,
        ...
        review_content = {
            "title": self._text(review.get("title")),
            "summary": self._text(review.get("summary")),
            "strength": self._textish(review.get("strength")),
            "weakness": self._textish(review.get("weakness")),
        }
        ratings = {
        ...
        prechecks: list[dict[str, str]] = [
            {
                ...
                "status": "pass",
                "reason": (
                    f"submitted_by=1, step={self._parse_int(review.get('step'))}, "
                    f"status_gate={'test override used' if test_mode else 'pending only'}"
                ),
            },
            {
                "name": "Required Review Fields",
                "status": "pass",
                "reason": "title, summary, strength, weakness, and all four ratings are present and usable",
            },
            {
                "name": "Ratings Range",
                "status": "pass",
                "reason": ", ".join(
                    ...
                    for label, value in (
                        ...
                        ("features_functionality", ratings["features_functionality"]),
        ...
        if self._narrative_sections_look_duplicate(review_content["strength"], review_content["weakness"]):
            prechecks.append(
                {
                    "name": "Strength vs Weakness Separation",
                    "status": "fail",
                    "reason": "strength and weakness are too similar or overlapping to be treated as clearly distinct sections",
                }
            )
        else:
            prechecks.append(
                {
                    "name": "Strength vs Weakness Separation",
                    "status": "pass",
                    "reason": "strength and weakness are materially different sections",
                }
            )
    ...
    def _render_markdown(self, payload: dict[str, Any]) -> str:
        ...
        lines = [
            ...
            "",
            "## Software",
            f"- Name: {self._fallback(subject['name'])}",
            f"- Categories: {', '.join(subject['category_labels']) if subject['category_labels'] else 'not available'}",
            "",
            "## Backend Prechecks",
        ]
        lines.extend(
        ...
        lines.extend(
            ...
            "",
            "## Review",
            f"- Title: {self._fallback(review_content['headline'])}",
            f"- Summary: {self._fallback(review_content['body'])}",
            f"- Strength: {self._fallback(review_content['strength'])}",
            f"- Weakness: {self._fallback(review_content['weakness'])}",
            "- Ratings:",
            f"  - Ease of Use: {self._fallback(review_content['ratings']['ease_of_use'])}",
            f"  - Features and Functionality: {self._fallback(review_content['ratings']['features_functionality'])}",
            f"  - Customer Support: {self._fallback(review_content['ratings']['customer_support'])}",
            f"  - Overall: {self._fallback(review_content['ratings']['overall'])}",
            "",
            "## Usage",
            f"- Duration Value: {self._fallback(usage['duration_value'])}",
            f"- Duration Unit: {self._fallback(usage['duration_unit'])}",
            f"- Frequency: {self._fallback(usage['frequency'])}",
            f"- Pricing: {self._fallback(usage['pricing'])}",
            f"- Integrates Other Software: {self._fallback(usage['integrates_other_software'])}",
            f"- Integrated Software: {', '.join(usage['integrated_software']) if usage['integrated_software'] else 'not provided'}",
            f"- Switched From Other Software: {self._fallback(usage['switched_from_other_software'])}",
            f"- Used Software Before Switch: {', '.join(usage['used_software_before_switch']) if usage['used_software_before_switch'] else 'not provided'}",
            "",
            "## Signals",
...
You are reviewing a client review submitted for a software product on the GoodFirms review platform.
...
"""
    ...
    def _iso_from_unix(self, value: Any) -> str | None:
        timestamp = self._parse_int(value)
    ...
    def _resolve_name_list(self, raw_value: Any, names_by_id: dict[str, str]) -> list[str]:
        values = self._read_string_list(raw_value)
        return [names_by_id.get(value, value) for value in values]

    def _read_string_list(self, raw_value: Any) -> list[str]:
        if not isinstance(raw_value, list):
            return []
        return [self._text(item) for item in raw_value if self._text(item)]

    def _textish(self, raw_value: Any) -> str:
        if isinstance(raw_value, str):
            return self._text(raw_value)
        if isinstance(raw_value, list):
            values = [self._text(item) for item in raw_value if self._text(item)]
            return "; ".join(values)
        if isinstance(raw_value, dict):
            values = [self._text(item) for item in raw_value.values() if self._text(item)]
            return "; ".join(values)
        return self._text(raw_value)
    ...
    def _parse_int(self, raw_value: Any) -> int | None:
        text_value = self._text(raw_value)
        if not text_value:
            return None
        try:
    ...
    def _extract_email_domain(self, value: str | None) -> str:
    ...
    def _fallback(self, value: Any) -> str:
        ...
        return text_value or "not provided"
